...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// --- RASPBERRY PI CHAOS SHOTGUN ---
	// Lightweight but WILD - throws everything at the wall
	
	// Minimal state (Pi-friendly)
	static uint32_t chaos_state = 0x9E3779B9;  // Golden ratio
	static uint32_t last_good = 0;
	static uint32_t best_dist = 0xFFFFFFFF;
	static uint8_t lucky_byte = 0;
	static uint32_t shot_pattern = 0;
	static uint32_t last_n = 0;
	
	// Super lightweight "physics" (just one number)
	static uint16_t cy_volume = 1000;  // Calabi-Yau volume * 1000
	...
	do {
		...
		if (distance < best_dist) {
			best_dist = distance;
			last_good = n;
			lucky_byte = hash[0] & 0xFF;
			cy_volume = (cy_volume + ((hash[1] >> 8) & 0x7FF)) >> 1;
		}
		
		// --- SHOTGUN BLAST PATTERN ---
		// Fire in all directions using cheapest operations
		
		uint32_t fb = swab32(hash[0]);
		
		// Evolve chaos state (ultra-cheap PRNG)
		chaos_state = chaos_state * 1103515245 + 12345;
		
		// Shot pattern (rotate through 8 patterns)
		shot_pattern = (shot_pattern + 1) & 0x7;
		...
		switch (shot_pattern) {
			case 0: // XOR chaos
				n ^= fb;
				blast = (chaos_state & 0xFFF) - 0x800;
				break;
				
			case 1: // Golden ratio jump
				blast = ((fb * 0x9E3779B9) >> 16) & 0xFFFF;
				break;
				
			case 2: // Prime-ish hop
				blast = (fb % 2053) + (chaos_state % 509);
				break;
				
			case 3: // Fibonacci
				{
					static uint32_t fib_a = 1, fib_b = 1;
					uint32_t fib_c = fib_a + fib_b;
					fib_a = fib_b;
					fib_b = fib_c;
					if (fib_b > 0x100000) { fib_a = 1; fib_b = 1; }
					blast = fib_b;
				}
				break;
				
			case 4: // CY volume target (String Theory winner!)
				{
					uint16_t hash_cy = ((hash[1] >> 8) & 0x7FF);
					int32_t cy_error = cy_volume - hash_cy;
					blast = cy_error * 2;
				}
				break;
				
			case 5: // Cluster echo (revisit good spot)
				if (best_dist < 0x40000000) {
					blast = ((int32_t)last_good - (int32_t)n) >> 2;
				} else {
					blast = (chaos_state & 0x3FFF) - 0x2000;
				}
				break;
				
			case 6: // Lucky byte pattern
				blast = (lucky_byte * fb) >> 8;
				n ^= (lucky_byte << 16);
				break;
				
			case 7: // Wild card - throw the dice
				blast = ((fb ^ chaos_state) % 32768) - 16384;
				n = (n * 0x9E3779B9) ^ fb;
				break;
		}
		...
		// Forward progress
		if (n <= last_n) {
			n = last_n + ((chaos_state & 0xFF) + 1);
		}
		last_n = n;
		
		// Ultra-lightweight reset (every 256K)
		if ((n & 0x3FFFF) == 0) {
			if (best_dist > 0xC0000000) {
				// Not finding anything - randomize
				chaos_state ^= fb;
				cy_volume = 500 + (fb & 0x7FF);
			}
			best_dist = (best_dist >> 1) | 0x80000000;
		}
