...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// --- String Theory v2: Supersymmetric Vein Mining ---
	// Combines String Theory's winning formula with vein detection
	...
	// --- VEIN MINING ADDITIONS ---
	
	// Elite results tracking
	#define VEIN_SIZE 32
	static uint32_t vein_locations[VEIN_SIZE] = {0};
	static uint32_t vein_distances[VEIN_SIZE] = {0xFFFFFFFF};
	static uint32_t vein_cy_volumes[VEIN_SIZE] = {0};  // CY volume at success
	...
	static uint32_t mining_mode = 0;  // 0=explore, 1=mine, 2=hyperfocus
	static uint32_t mode_timer = 0;
	
	// Best configuration ever found
	static uint32_t best_distance = 0xFFFFFFFF;
	static double best_cy_volume = 1.0;
	static double best_coupling = 0.1;
	static double best_dilaton = -2.3;
	...
	do {
		...
		// --- Extract Spacetime Configuration ---
		...
		// --- Calabi-Yau Volume (CRITICAL PARAMETER) ---
		...
		if (distance < quality_threshold) {
			...
			vein_cy_volumes[vein_idx] = (uint32_t)(calabi_yau_volume * 1000.0);
			...
			if (distance < best_distance) {
				...
				best_cy_volume = calabi_yau_volume;
				best_coupling = coupling_gs;
				best_dilaton = dilaton_field;
				best_location = n;
				
				// Trigger hyperfocus mode
				mining_mode = 2;
				mode_timer = 0;
			}
		}
		...
		if (window_total > 1000) {
			...
			if (mining_mode == 0 && hit_rate > 0.003 && hits_in_window >= 2) {
				mining_mode = 1;  // Enter mining mode
				mode_timer = 0;
			}
			
			if (mining_mode == 1 && hit_rate > 0.006 && hits_in_window >= 4) {
				mining_mode = 2;  // Upgrade to hyperfocus
				mode_timer = 0;
			}
		...
		// Mode timeout
		if (mining_mode == 1 && mode_timer > 5000) mining_mode = 0;
		if (mining_mode == 2 && mode_timer > 8000) mining_mode = 0;
		...
		if (mining_mode == 2) {
			// HYPERFOCUS MODE - mine around best known configuration
			
			// Lock onto best CY volume
			double volume_target = best_cy_volume;
			double volume_error = calabi_yau_volume - volume_target;
			
			int64_t volume_correction = -(int64_t)(volume_error * 2048.0);
			perturbation += volume_correction;
			
			// Stay near best location
			int64_t location_pull = ((int64_t)best_location - (int64_t)n) >> 3;
			perturbation += location_pull;
			
			// Micro-search pattern
			uint32_t micro_pattern = (mode_timer % 64);
			perturbation += (micro_pattern - 32);
			
			// Use best dilaton configuration
			if (fabs(dilaton_field - best_dilaton) > 0.5) {
				dilaton_field = dilaton_field * 0.7 + best_dilaton * 0.3;
			}
			
		} else if (mining_mode == 1) {
			// MINING MODE - exploit vein patterns
			
			// Calculate average CY volume in vein
			double avg_cy_volume = 0.0;
			for (int i = 0; i < vein_count; i++) {
				avg_cy_volume += (double)vein_cy_volumes[i] / 1000.0;
			}
			if (vein_count > 0) avg_cy_volume /= vein_count;
			
			// Target that volume
			double volume_error = calabi_yau_volume - avg_cy_volume;
			perturbation += -(int64_t)(volume_error * 1024.0);
			
			// Jump between vein locations
			if ((mode_timer & 0x1F) == 0 && vein_count > 0) {
				uint32_t target_idx = hash[3] % vein_count;
				perturbation += ((int64_t)vein_locations[target_idx] - (int64_t)n) >> 2;
			}
			
		} else {
			// EXPLORATION MODE - full string theory dynamics
		...
		// Winding
		if (winding_quantum > 0) {
			perturbation += winding_quantum * 256;
		}
		...
		// Supersymmetry preservation bonus
		if (susy_preserved) {
			perturbation += 384;
		}
		...
		if ((n & 0x7FFF) == 0) {
			...
			// Decay best if not in mining mode
			if (mining_mode == 0) {
