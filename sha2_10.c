...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// 303X1 - Radar - Pell numbers sequencing
	// Pell numbers: P(n) = 2*P(n-1) + P(n-2), P(0)=0, P(1)=1
	...
	do {
		...
		uint32_t pell_idx = (fb >> 8) & 0xF;  // Use bits 8-11 for index (0-15)
		uint32_t pell_step = pell_table[pell_idx];
		
		n += pell_step;
		
		// Ensure n stays odd (important for prime-related searches)
		n |= 1;
