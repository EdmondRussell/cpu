...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// 303X1 - Radar - Low-Discrepancy State-Space Traversal
	// 
	// HYPOTHESIS: Under the random oracle model, SHA-256d(block || n) output
	// is uniformly distributed. Linear incrementation creates sequential
	// compression states with high correlation in early rounds. A low-discrepancy
	// sequence maximizes state-space coverage with minimal clustering.
	//
	// STRATEGY: Weyl sequence (additive recurrence) using irrational constant
	// guarantees equidistribution by the three-distance theorem. This minimizes
	// revisiting similar compression trajectories in the first 16 rounds where
	// the message schedule is deterministic.
	//
	// TESTABLE METRICS:
	// 1. Chi-squared test on nonce distribution
	// 2. Avalanche distance between consecutive compression states
	// 3. Cache efficiency (prefetch hit rate)
	// 4. Solutions found per unit time vs linear scan
	
	// Weyl sequence constant: floor(2^32 / φ) where φ = golden ratio
	// This is optimal for 1D equidistribution (Knuth, TAoCP Vol 3)
	static const uint32_t WEYL_CONST = 0x9E3779B9U;
	
	// Thread-local state to avoid correlation between workers
	static __thread uint32_t weyl_state = 0;
	static __thread uint32_t init_done = 0;
	
	// Initialize Weyl state from thread ID and timestamp for decorrelation
	if (!init_done) {
		weyl_state = (uint32_t)thr_id * 2654435761U + first_nonce;
		init_done = 1;
	}
	...
	// Benchmarking checkpoint
	uint32_t iterations = 0;
	
	do {
		// Weyl sequence: guarantees full coverage of 32-bit space
		// with minimal local clustering (star discrepancy O(log N / N))
