...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// 303X1 - Radar - Predictive Entropy Targeting
	// Uses hash proximity analysis and adaptive gradient descent
	static uint32_t history[8] = {0};  // Track recent hash[7] values
	static uint8_t hist_idx = 0;
	static int32_t momentum = 0;
	...
	do {
		...
		uint32_t distance = h7_swapped - Htarg;
		
		// Store in circular history buffer
		history[hist_idx] = h7_swapped;
		hist_idx = (hist_idx + 1) & 7;
		
		// Analyze trend: are we getting closer?
		int32_t gradient = 0;
		if (distance < best_distance) {
			gradient = -1;  // Getting closer (good)
			best_distance = distance;
		} else {
			gradient = 1;   // Getting farther (bad)
		}
		
		// Update momentum with decay
		momentum = (momentum * 3) / 4 + gradient;
		
		// Extract entropy from current hash
		uint32_t entropy = hash[0] ^ hash[1] ^ hash[2] ^ hash[3];
		entropy ^= hash[4] ^ hash[5] ^ hash[6] ^ hash[7];
		
		// Compute statistical variance across history
		uint64_t mean = 0;
		for (int i = 0; i < 8; i++) {
			mean += history[i];
		}
		...
		uint64_t variance = 0;
		for (int i = 0; i < 8; i++) {
			int64_t diff = (int64_t)history[i] - (int64_t)mean;
			variance += diff * diff;
		}
		variance >>= 3;
		
		// Adaptive step calculation based on multiple factors
		uint32_t base_step;
		
		if (distance > 0x10000000) {
			// Very far: use large golden-ratio jumps
			base_step = ((uint64_t)entropy * 0x9E3779B9ULL) >> 24;
		} else if (distance > 0x1000000) {
			// Moderately far: medium prime-like steps
			base_step = (entropy & 0xFFFF) | 1;
		} else {
			// Close: fine-grained search with gradient hints
			base_step = (entropy & 0xFF) + 1;
		}
		
		// Apply momentum-based correction
		if (momentum < -2) {
			// Strong positive trend: keep going in same direction
			base_step = base_step;
		} else if (momentum > 2) {
			// Strong negative trend: reverse or explore
			base_step = ~base_step;
		}
		
		// Variance-driven exploration vs exploitation
		if (variance < 0x100000000ULL) {
			// Low variance: we're stuck, need exploration
			base_step ^= (entropy >> 16);
			base_step *= 1103515245;  // LCG multiplier for chaos
		}
		
		// Bit-level target alignment heuristic
		// Find differing bit positions between current and target
		uint32_t diff_bits = h7_swapped ^ Htarg;
		uint32_t highest_diff_bit = 31 - __builtin_clz(diff_bits | 1);
		
		// Weight step by bit significance
		uint32_t bit_weight = 1U << (highest_diff_bit / 2);
		base_step = (base_step & (bit_weight - 1)) + bit_weight;
		
		// XOR feedback from hash for chaos injection
		n ^= (hash[0] & 0xFF);
		
		// Apply calculated step
		n += base_step;
		
		// Ensure odd (prime-friendly)
		n |= 1;
		
		// Occasional random reset to escape local minima
		if ((n & 0xFFFF) == 0xFFFF) {
			n ^= entropy;
