                     ...
                     uint32_t max_nonce, uint64_t *hashes_done)
{
    ...
    /* Pell numbers (used only as entropy, not nonce jumps) */
    static const uint32_t pell_table[16] = {
    ...
    do {
        ...
        /*
         * Hash-derived perturbation (POOL-SAFE):
         * Mutate an unused word in the message schedule
         * without affecting nonce monotonicity.
         */
        uint32_t fb = hash[0];
        uint32_t pell_idx  = (fb >> 8) & 0xF;
        uint32_t pell_step = pell_table[pell_idx];

        /* Mix entropy into message extension area */
        data[8] ^= pell_step;
