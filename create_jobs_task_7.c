...
// Large prime stride — covers nonce space in wide, evenly distributed jumps.
// Coprime to 2^32 so it produces a full-period sequence across all 4B nonces.
#define GUIDED_STRIDE 0x9E3779B9U  // golden ratio constant, guarantees full coverage

static inline uint32_t guided_step(uint32_t last)
{
    // xorshift32 entropy mix — used to occasionally perturb the stride
    // so repeated jobs on the same work don't retrace identical paths
    guided_entropy ^= guided_entropy << 13;
    guided_entropy ^= guided_entropy >> 17;
    guided_entropy ^= guided_entropy << 5;

    // Primary advance: large golden-ratio stride across full nonce space
    uint32_t next = last + GUIDED_STRIDE;

    // Small entropy perturbation every ~16 steps to avoid stride aliasing
...
#define PACE_TARGET_LATENCY_MS  30      // tune to your chip's FIFO depth
...
void create_jobs_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (new_mining_notification != NULL) {
            ...
            // Previous version guided_entropy ^= guided_nonce;
            guided_entropy ^= guided_nonce ^ (uint32_t)esp_timer_get_time();  // add timer for extra entropy
        ...
        guided_nonce = (uint32_t)(extranonce_2 * 0x9E3779B9U) ^ guided_entropy;
