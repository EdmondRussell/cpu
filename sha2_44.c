...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// --- String Theory v6: Champion's Twin ---
	// Slight variation on the winning formula
	...
	// String dynamics
	static double string_tension = 1.2;  // Slightly higher initial tension
	static double coupling_gs = 0.12;    // Slightly different coupling
	static double dilaton_field = -2.1;  // Slightly different dilaton
	static double worldsheet_area = 0.0;
	
	// Calabi-Yau manifold (THE WINNER)
	static double calabi_yau_volume = 1.1;  // Different starting volume
	...
	do {
		...
		// --- Calabi-Yau Volume (CRITICAL WINNER) ---
		...
		// Calabi-Yau volume guidance (THE KEY)
