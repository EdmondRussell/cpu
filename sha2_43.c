...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// --- String Theory v5: Simplified Powerhouse ---
	// Keep what works, remove complexity
	...
	// Calabi-Yau geometry (PROVEN CRITICAL)
	...
	static uint32_t good_locations[MEMORY_SIZE] = {0};
	static uint32_t good_distances[MEMORY_SIZE] = {0xFFFFFFFF};
	static double good_cy_volumes[MEMORY_SIZE] = {0};
	static uint32_t memory_idx = 0;
	...
	static uint32_t iterations_since_good = 0;
	static uint32_t focused_search = 0;
	...
	do {
		...
		iterations_since_good++;
		
		// --- EXTRACT 11D SPACETIME ---
		...
		// --- CALABI-YAU VOLUME (KEY PARAMETER) ---
		...
		uint32_t good_threshold = 0x20000000;
		
		if (distance < good_threshold) {
			// Store in memory
			good_locations[memory_idx] = n;
			good_distances[memory_idx] = distance;
			good_cy_volumes[memory_idx] = calabi_yau_volume;
			memory_idx = (memory_idx + 1) % MEMORY_SIZE;
			
			iterations_since_good = 0;
		...
		if (focused_search > 0) {
			...
			// Pull toward best location
			int64_t best_pull = ((int64_t)best_location - (int64_t)n) >> 2;
			perturbation += best_pull;
			
			// Target best CY volume
			double volume_error = calabi_yau_volume - best_cy_volume;
			perturbation += -(int64_t)(volume_error * 1024.0);
			
			// Target best coupling
			double coupling_error = coupling_gs - best_coupling;
			perturbation += -(int64_t)(coupling_error * 768.0);
			
			// Small random variations
			perturbation += ((hash[2] % 512) - 256);
			
			// Jump to remembered good locations occasionally
			if ((focused_search & 0x1F) == 0) {
				uint32_t target_idx = hash[3] % MEMORY_SIZE;
				if (good_distances[target_idx] < 0x80000000) {
					int64_t target_pull = ((int64_t)good_locations[target_idx] - (int64_t)n) >> 1;
					perturbation += target_pull;
					
					// Also target that CY volume
					double target_cy = good_cy_volumes[target_idx];
					volume_error = calabi_yau_volume - target_cy;
					perturbation += -(int64_t)(volume_error * 512.0);
				}
			}
			...
			// Standard string theory dynamics
			...
			// If we haven't found anything good in a while, explore more aggressively
			if (iterations_since_good > 10000) {
				perturbation += (hash[1] % 8192) - 4096;
			}
			
			// Occasionally revisit good locations
			if ((iterations_since_good & 0xFF) == 0 && good_distances[0] < 0x80000000) {
				uint32_t revisit_idx = (iterations_since_good >> 8) % MEMORY_SIZE;
				if (good_distances[revisit_idx] < 0x80000000) {
					int64_t revisit_pull = ((int64_t)good_locations[revisit_idx] - (int64_t)n);
					perturbation += (revisit_pull >> 3);
				}
			}
		}
		
		// --- UNIVERSAL COMPONENTS (ALWAYS ACTIVE) ---
		...
		if ((n & 0xFFFF) == 0) {
			...
			// Decay best if not finding anything
			if (iterations_since_good > 50000 && !focused_search) {
				best_distance = (best_distance >> 1) | 0x80000000;
			}
			
			// Decay memory
			for (int i = 0; i < MEMORY_SIZE; i++) {
				if (good_distances[i] < 0xFFFFFFFF) {
					good_distances[i] = (good_distances[i] >> 1) | 0x40000000;
				}
			}
			
			// Reset focused search if it's been too long
			if (focused_search > 0 && iterations_since_good > 5000) {
				focused_search = 0;
			}
		}
