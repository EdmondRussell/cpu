...
int scanhash_sha256d(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	// 303X1 - Radar - Golden ratio hashing
	// φ ≈ 1.618033988749895 (golden ratio)
	// Using fixed-point: φ * 2^32 ≈ 6943656394 (for integer math)
	// Multiplicative hashing with golden ratio for uniform distribution
	...
	do {
		...
		// This creates pseudo-random but evenly distributed jumps
		uint64_t step = ((uint64_t)fb * PHI_32) >> 32;
		
		// Scale step to reasonable range (avoid tiny or huge jumps)
		step = (step & 0xFFFF) + 1;  // Range: 1 to 65536
		
		n += (uint32_t)step;
		
		// Ensure n stays odd for prime-related properties
		n |= 1;
